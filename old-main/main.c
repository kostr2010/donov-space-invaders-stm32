...
uint32_t pins_used = DIOD_E | DIOD_D | DIOD_DP | DIOD_C | DIOD_G | DIGIT4 | DIOD_B | DIGIT3 |
...
void rcc_config(void) {
  ...
  LL_RCC_HSI_Enable(); // enables internal RC oscillator in order to set frequency == 48 MHz (which
                       // is operational frequency of the periferials)
  LL_RCC_PLL_Enable(); // enables phase locked loop in order to set frequency == 48 MHz (which is
                       // operational frequency of the periferials)
  while (LL_RCC_HSI_IsReady() != 1 && LL_RCC_PLL_IsReady() != 1)
    ; // waits until flags are updated

  LL_RCC_PLL_ConfigDomain_SYS(
      LL_RCC_PLLSOURCE_HSI_DIV_2,
      LL_RCC_PLL_MUL_12); // Configure PLL used for SYSCLK(systemclock) Domain.

  LL_RCC_SetAHBPrescaler(
      LL_RCC_SYSCLK_DIV_1); // do not change AHB bus's(DMA, RCC, all of the GPIO's, etc) frequency
  LL_RCC_SetSysClkSource(
      LL_RCC_SYS_CLKSOURCE_PLL); // setting configured PLL as the core clock's source
  ...
  SystemCoreClock =
      48000000; // the SystemCoreClock CMSIS variable is used to store System Clock Frequency
...
void gpio_config(void) {
  ...
  LL_AHB1_GRP1_EnableClock(LL_AHB1_GRP1_PERIPH_GPIOC); // enable AHB1 peripherals clock for B pins
                                                       // (setting port's frequency, it's vital!)
...
void set_indicator_hp(int hp, int tick); // sets indicator for hp in format 'HP-x', where x stands
                                         // for amount of hp left. only supports 1 digit numbers
int get_encoder_rotation(); // gets the encoder rotation. returns -1 if rotating left and 1 if
                            // rotating right
void printf_config(void);   // configuration for oled display printf
__attribute__((naked)) void
delay_10ms(void); // just set of commands to waste CPU power for 10ms (basically it is a simple
                  // cycle with a predefined number of loops)                             //
                  // function that handles the button press. spawns one player's missle
int mypow(int x, int y); // elevates x to power y                                             //
                         // simulates the real one
// void print_battlefield(entity* battlefield);                          // prints the battlefield
// in terminal entity* add_ship(entity* battlefield,entity ship, int x, int y);      // adds new ship
// to the battlefield, rezises the battlefield itself entity* remove_ship(entity* battlefield, entity
// ship);                // removes ship from the battlefield, changes the batllefield's size, clozes
// the gaps in the array
...
// entity* spawn_player(entity* battlefield);                            // function that handles
// the button press. spawns one player's missle
...
entity* update_entities(entity* battlefield) {
  ...
  if (battlefield[0].health <= 0 || battlefield[0].pos_y <= 0 || battlefield[0].pos_y >= 64 ||
      battlefield[0].pos_x <= 0 || battlefield[0].pos_x >= 128) {
    defeat_flag = 1;
  }

  for (uint8_t i = 1; i < cpy; i++) {
    if (battlefield[i].health <= 0 || battlefield[i].pos_y <= 0 || battlefield[i].pos_y >= 64 ||
        battlefield[i].pos_x <= 0 || battlefield[i].pos_x >= 128) {
...
entity* spawn_enemies(entity* battlefield, int tick) {
    ...
    for (uint8_t i = 0; i < entities_on_field; i++) {
      if (battlefield[i].iff_flag == 'e' && battlefield[i].entity_flag == 's') {
        ...
        battlefield[entities_on_field - 1].mov_x =
            (battlefield[0].pos_x - battlefield[entities_on_field - 1].pos_x) / 15;
        battlefield[entities_on_field - 1].mov_y =
            (battlefield[0].pos_y - battlefield[entities_on_field - 1].pos_y) / 15;
      }
    }
  }
  ...
  if (first_row_free) {
    ...
    for (uint8_t i = entities_on_field - 6; i < entities_on_field; i++) {
      battlefield[i] = corvette;
      battlefield[i].pos_x =
          corvette.pos_x + (-entities_on_field + 6 + i + 1) * (corvette.width + 8);
    }
  }
