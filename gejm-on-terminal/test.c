...
entity player_ship = {PLAYER_SHIP_DMG, 10, 10, PLAYER_SHIP_LENGTH, PLAYER_SHIP_WIDTH, 9, PLAYER_SHIP_IFF, PLAYER_SHIP_ENTITY, 0, 0};
entity player_projectile = {PLAYER_PROJECTILE_DMG, 0, 0, PLAYER_PROJECTILE_LENGTH, PLAYER_PROJECTILE_WIDTH, 1, PLAYER_PROJECTILE_IFF, PLAYER_PROJECTILE_ENTITY, 4, 0};
entity corvette = {CORVETTE_DMG, 58, 10, CORVETTE_LENGTH, CORVETTE_WIDTH, 5, CORVETTE_IFF, CORVETTE_ENTITY, -1, -2};
entity corvette_projectile = {CORVETTE_PROJECTILE_DMG, 0, 0, CORVETTE_PROJECTILE_LENGTH, CORVETTE_PROJECTILE_WIDTH, 1, CORVETTE_PROJECTILE_IFF, CORVETTE_PROJECTILE_ENTITY, -4, 0};
...
void print_battlefield(entity* battlefield) {
    for (int i = 0; i < entities_on_field; i++) {
        printf("ship number: %d\n damage: %d; coordinates y: %d; coordinates x: %d; health: %d; friend or enemy: %c; ship or projectile: %c \n \n", i, battlefield[i].damage, battlefield[i].pos_y, battlefield[i].pos_x, battlefield[i].health, battlefield[i].iff_flag, battlefield[i].entity_flag);
    }
}
...
entity* update_entities(entity* battlefield) {
    ...
    for (int i = 0; i < cpy; i++) {
        for (int j = i + 1; j < cpy; j++) {
            if ((abs(battlefield[i].pos_x - battlefield[j].pos_x) <= (battlefield[j].width + battlefield[i].width)/2)
            && (abs(battlefield[i].pos_y - battlefield[j].pos_y) <= (battlefield[j].length + battlefield[i].length)/2)
    ...
    for (int i = 1; i < cpy; i++) {
        ...
        if (battlefield[i].health <= 0
            || battlefield[i].pos_x <= 0
            || battlefield[i].pos_x >= 64
            || battlefield[i].pos_y <= 0
            || battlefield[i].pos_x >= 128 ) {
...
entity* spawn_enemies(entity* battlefield, int tick) {
    ...
    if (tick % 50 == 0) {
        for (int i = 0; i < entities_on_field; i++) {
            if (battlefield[i].iff_flag == 'e' && battlefield[i].entity_flag == 's') {
                ...
                battlefield[entities_on_field - 1].mov_x = (battlefield[0].pos_x - battlefield[entities_on_field - 1].pos_x) / 10;
                battlefield[entities_on_field - 1].mov_y = (battlefield[0].pos_y - battlefield[entities_on_field - 1].pos_y) / 10;
            }
        }
    }
    ...
    if (first_row_free) {
        ...
        for (int i = entities_on_field - 3; i < entities_on_field; i++) {
            battlefield[i] = corvette;
            battlefield[i].pos_x = corvette.pos_x + (-entities_on_field + 3 + i + 1)*(corvette.width + 8);
        }
    }
